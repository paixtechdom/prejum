...
const Card = ({header, text, img, buttonText, buttonLink, clas, w, buttonIcon}) => {
    return(
        <div className="flex justify-center items-center w-full text-gray-200 py-9 md:h-96 my-9">
...
const CardImageOverlay = ({header, text, img, buttonText, buttonLink, w}) => {
    return(
        <div className="flex justify-center items-center w-full text-gray-200 py-9 h-96 my-9 relative">
            ...
            <div className={`flex ${!w == '' ? `md:w-9/12 w-11/12` : 'md:w-8/12 w-10/12'}  items-center h-full`}>
                <div className="flex flex-col md:gap-5 gap-3 items-center text-center md:items-start md:text-left md:w-7/12 z-10">
                    ...
                    <Link to={`/${buttonLink}`}>
                        <SecondaryButton text={buttonText} icon={'arrow-right'}/>
                    </Link>
                </div>
            </div>
        </div>
